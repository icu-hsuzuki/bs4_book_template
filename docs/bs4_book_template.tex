...
\hypersetup{
  pdftitle={BS4 Book Template},
  pdfauthor={Hiroshi Suzuki},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{BS4 Book Template}
...
\hypertarget{ux65e5ux672cux8a9eux7248ux30c6ux30f3ux30d7ux30ecux30fcux30c8ux306bux3064ux3044ux3066}{%
\subsection{日本語版テンプレートについて}\label{ux65e5ux672cux8a9eux7248ux30c6ux30f3ux30d7ux30ecux30fcux30c8ux306bux3064ux3044ux3066}}

\href{https://github.com/icu-hsuzuki/bs4_book_template}{GitHub レポジトリ} の、README を参照してください。
...
\item
  README.md を、ご自分の本にあうように書き換えてください。このテンプレートのリポジトリに、README はありますので、必要な時は参照してください。
\item
  \texttt{bookdown.yml} の最初の行、\texttt{book\_filename:\ "bs4\_book\_template"} を、書き換えてください。現在は、リポジトリの名前になっています。
\item
  \texttt{output.yml} の５行目の、\texttt{repo:\ https://github.com/icu-hsuzuki/bs4\_book\_template} を、ご自分のリポジトリのアドレスに変更して下さい。\texttt{bs4\_book} のスタイルでは、このアドレスが、左の帯の一番下に表示され、リポジトリにリンクされます。
\item
  \texttt{index.html} には、YAML と呼ばれる、設定が、書かれていますが、まず、title と author がありますから、それらを、ご自身の本のタイトルと、ご自身の名前に変更してください。
\item
  \texttt{url} と \texttt{cover-image} が、コメントアウトされています。\texttt{url} は、リポジトリのアドレスではなく、公開される、サイトのアドレスです。\texttt{cover-image} はなければ、加える必要はありません。
\item
  \texttt{description} を、ご自身の本にあう形に、書き換えてください。
\item
  index.Rmd と、01-intro.Rmd から書き換えていってください。大枠が決まるまでは、わたしは、01-intro.Rmd, 11-part2.Rmd などと、すこし、間をあけて、番号をふったファイルを作り、書いていっています。章番号は、順番は変えずに、自動的に続き番号になります。むろん、あとから、変更することも可能ですが、\texttt{docs} 内に、さまざまなファイルが生成されますので、ある程度、計画的に、ファイルを作成していった方がよいように思います。
\end{enumerate}
...
\item
  \texttt{bookdown} の本に何度も書かれていますが、PDF や、ePub は、ページの概念があり、複雑ですから、まず、基本的な内容を書き終わるまでは、output.yml の bookdown::pdf\_book 以下の行をすべて、コメントアウトし（RStudio の場合は、ハイライトさせ、メニューバーの Code の Comment/Uncomment Lines を使えば、簡単にできます。）Book の build の時間も短くなります。
\item
  PDF を作成するときには、必要に応じて、\texttt{preamble.tex} に、情報を加えたり、フォントを追加してください。
\end{itemize}
...
\subsubsection{A section}\label{a-section-1}}

All chapter sections start with a second-level (\texttt{\#\#}) or higher heading followed by your section title, like the sections above and below here. You can have as many as you want within a chapter.

\hypertarget{an-unnumbered-section-1}{%
\paragraph*{An unnumbered section}\label{an-unnumbered-section-1}}
